...
func getField(v *jira.IssueFields, field string) string {
	r := reflect.ValueOf(v)
	fieldValue := r.FieldByName(field)
	return fieldValue.Interface().(string)
}

func (p *Plugin) matchesSubsciptionFilters(wh *webhook, filters SubscriptionFilters, instanceID types.ID, MattermostUserID string) bool {
	...
	if len(filters.Self) > 0 {
		c, err := p.userStore.LoadConnection(instanceID, types.ID(MattermostUserID))
		if err == nil {
			for _, filterSelf := range filters.Self {
				switch filterSelf {
				case "reporter":
					if issue.Fields.Reporter.Self == c.User.Self {
						return true
					}
				case "assignee":
					if issue.Fields.Assignee != nil {
						if issue.Fields.Assignee.Self != "" && issue.Fields.Assignee.Self == c.User.Self {
							return true
						}
					}
				case "watcher":
					instance, errI := p.instanceStore.LoadInstance(instanceID)
					if errI == nil {
						client, errC := instance.GetClient(c)
						if errC == nil {
							meIssue, errIs := client.GetIssue(issue.Key, nil)
							if errIs == nil {
								if meIssue.Fields.Watches.IsWatching {
									return true
								}
							}
						}
...
func (p *Plugin) getChannelsSubscribed(wh *webhook, instanceID types.ID) ([]ChannelSubscription, error) {
	...
	for _, sub := range subIds {
		if p.matchesSubsciptionFilters(wh, sub.Filters, instanceID, sub.MattermostUserID) {
...
func (p *Plugin) validateSubscription(instanceID types.ID, subscription *ChannelSubscription, client Client) error {
	...
	if (len(subscription.Filters.Self)) == 0 {
		return errors.New("please provide a issue role")
	}
...
func (p *Plugin) httpChannelCreateSubscription(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	direct, errDirect := p.API.GetDirectChannel(mattermostUserID, p.getUserID())
	if errDirect != nil {
		return respondErr(w, http.StatusInternalServerError, errDirect)
	}

	subscription.ChannelID = direct.Id

	client, _, _, err := p.getClient(subscription.InstanceID, types.ID(mattermostUserID))
	if err != nil {
		return respondErr(w, http.StatusInternalServerError, err)
	}
	...
	err = p.client.Post.CreatePost(&model.Post{
		...
		ChannelId: subscription.ChannelID,
		Message:   fmt.Sprintf("Jira subscription, \"%v\", was added", subscription.Name),
	})
	if err != nil {
...
func (p *Plugin) httpChannelEditSubscription(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	err = p.client.Post.CreatePost(&model.Post{
		UserId:    p.getConfig().botUserID,
		ChannelId: subscription.ChannelID,
		Message:   fmt.Sprintf("Jira subscription, \"%v\", was updated", subscription.Name),
	})
	if err != nil {
		return respondErr(w, http.StatusInternalServerError,
			errors.WithMessage(err, "failed to create notification post"))
...
func (p *Plugin) httpChannelDeleteSubscription(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	err = p.client.Post.CreatePost(&model.Post{
		UserId:    p.getConfig().botUserID,
		ChannelId: subscription.ChannelID,
		Message:   fmt.Sprintf("Jira subscription, \"%v\", was removed", subscription.Name),
	})
	if err != nil {
		return respondErr(w, http.StatusInternalServerError,
			errors.WithMessage(err, "failed to create notification post"))
...
func (p *Plugin) httpChannelGetSubscriptions(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	if errDirect != nil {
		return respondErr(w, http.StatusBadRequest,
			errors.WithMessage(errDirect, "failed create direct channel for Jira user"))
	}
	channelID := direct.Id

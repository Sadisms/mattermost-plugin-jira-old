...
func (wh *webhook) CheckPermissions(instanceID types.ID, p *Plugin, mattermostUserID string) error {
	var err error
	...
	c, err2 := p.userStore.LoadConnection(instance.GetID(), types.ID(mattermostUserID))
	if err2 != nil {
		return err
	}
	client, err2 := instance.GetClient(c)
	...
	isCommentEvent := wh.Events().Intersection(commentEvents).Len() > 0
	// TODO: Add parse comment for deleted comment
	if isCommentEvent && len(wh.Comment.ID) > 0 {
		if instance.Common().IsCloudInstance() {
			err = client.RESTGet(fmt.Sprintf("/2/issue/%s/comment/%s", wh.Issue.ID, wh.Comment.ID), nil, &struct{}{})
		} else {
			err = client.RESTGet(wh.JiraWebhook.Comment.Self, nil, &struct{}{})
		}
	}
